...
from control_with_coords import *
global ER
mutex = Lock()
...
if __name__== '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if len(virtual_angles)==6 and len(J_speed)!=0:
            ...
            for i in range(len(xyz_virtual)):
                xyz_virtual[i] = xyz_virtual[i] + 10*J_speed[i]
            print(xyz_virtual)
            virtual_check =coords_to_angels(xyz_virtual,virtual_angles)
            if len(virtual_check)>0:
                virtual_angles = virtual_check
                joint_state_virtual.position = [iter*math.pi/180 for iter in virtual_angles]
                joint_state_virtual.header.stamp = rospy.Time().now()
                pub_virtual.publish(joint_state_virtual)
        ...
        if B_press:
            xyz_check = ER.get_coords()
            angles_check = ER.get_angles()
            if check_angles(angles_check):
                real_angles = angles_check
            if check_angles(xyz_check):
                xyz_virtual = xyz_check
