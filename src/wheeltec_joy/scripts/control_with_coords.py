...
dDH = [200,0,0,107.97,109.14,76.2]  # 连杆距离d  units: mm
a1=0 
a2=0 
a3=250
a4=250
...
class Kinematic:
    ...
    def toangels(self,T):
          ...
          index5=0
          for x in theta1:
            print("theta5:",math.acos(r23*math.cos(x)-r13*math.sin(x))/PI*180,)
...
def coords_to_angels(coords,current_angels):        
    ...
    for i in range(0,len(theta_result)):
        ddiff=0
        for j in range(0,6):
            ddiff=weight[j]*abs(theta_result[i][j]-current_angels[j]/180*PI)+ddiff
        diff.append(ddiff)
